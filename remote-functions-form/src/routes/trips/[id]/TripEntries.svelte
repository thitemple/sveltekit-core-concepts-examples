<script lang="ts">
	...
	import { getTrip, getTripEntries } from '$lib/trips.remote';
  ...
  const entriesResult = $derived(await getTripEntries({tripId, page: String(currentPage)}));
	...
	const paginationCurrentPage = $derived(
		clampPage(currentPage, pagination?.totalPages),
	);
	...
	const addEntryDialogId = $derived(
		trip ? `add-entry-dialog-${trip.id}` : 'add-entry-dialog'
	);
